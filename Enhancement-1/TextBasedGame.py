...
# Harkamal Sandhu
# CS-499
# Enhanced version: With new rooms and Puzzles
# Enhancements as per instructor's feedback: get_item() method is documented properly with No redundancy
#                                             main_loop is documented with detailed comments
#################################################
...
rooms = {
        ...
        'Library': {'South': 'Reception', 'West': 'Emergency_Exit', 'item': 'Magic-Potion'},    # New Addition
        'Cafeteria': {'North': 'Reception', 'item': 'Keycard'},                                 # New Addition
        'Emergency_Exit': {'East': 'Library', 'item': 'Night-Vision-Glass'}                     # New Addition

}

# Puzzles that need to be solved to access certain rooms or items (# New Addition)
puzzles = {
...
# Correct answers for the puzzles (# New Addition)
puzzle_answers = {
        'Pharmacy': '65',
        'Surgical_Room': 'Paris',
        'Emergency_Exit': '4'
...
def show_instructions():
    ...
    print('Added option of Emergency_Exit')
...
def get_item(present_room, stock):
    # Checking room item has been collected or not
    if rooms[present_room]['item'] not in stock: # if not collected then proceed to get the item
        outcome = False
        while not outcome:
            command = input('Enter your move:').split() # Enter move to collect item
            move = command[0]
            ...
            item = command[1]
            if item == rooms[present_room]['item']: # Check whether input item matches room's item
                outcome = True
        ...
        # No command is of single word, it is either 'go direction' or 'get item'
            ...
            if move2 in rooms[current_room]:
                ...
                if rooms[current_room][move2] in puzzles:
                    print(puzzles[rooms[current_room][move2]])                  # Display puzzle question
                    answer = input()
                    if answer != puzzle_answers[rooms[current_room][move2]]:    # Match puzzle answer
                ...
                # Function Call
                show_status(current_room, inventory) # show current status

                if current_room == 'Basement' or current_room == 'Reception':      # Villain's Room or starting point (No item to collect)
                    continue
                elif current_room == 'Emergency_Exit':                             # Emergency Exit
